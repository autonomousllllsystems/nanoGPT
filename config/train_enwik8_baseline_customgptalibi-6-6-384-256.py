att_scores = (Q @ K^T) / sqrt(d) + alibi_slopes[h] * (i - j)  # Adds relative position bias# train a miniature character-level shakespeare model with ALiBi attention
...
dropout = 0.1  # Lower dropout - ALiBi is less prone to overfitting
bias = True  # Use bias for better expressiveness

out_dir =        f'out-enwik8-baseline-customgpt-alibi-{n_layer}-{n_head}-{n_embd}-{block_size}'
wandb_run_name = f'enwik8-baseline-customgpt-alibi-{n_layer}-{n_head}-{n_embd}-{block_size}'

learning_rate = 3e-4 # Optimal for Adam with this architecture
max_iters = 10000 # More iterations for convergence
